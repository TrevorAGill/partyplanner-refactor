    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if(introPrompt.equals("Get an estimate")) {
                ...
                newEvent.foodSelection = bufferedReader.readLine();
                int foodPrice = newEvent.getFoodPricePerGuest(newEvent.foodSelection);
                System.out.println("Select a beverage option: 'soda and water', 'domestic beers', 'wine and import beers'");
                newEvent.beverageSelection = bufferedReader.readLine();
                int beveragePrice = newEvent.getBeveragePricePerGuest(newEvent.beverageSelection);
                ...
                newEvent.numOfGuests = Integer.parseInt(bufferedReader.readLine());
                int totalFB = newEvent.getTotalGuestPriceExcludingEntertainment(fbPerGuest,newEvent.numOfGuests);
                ...
                newEvent.entertainmentSelection = bufferedReader.readLine();
                int entertainmentPrice = newEvent.getEntertainmentPrice(newEvent.entertainmentSelection);
                int totalPricePrediscount = newEvent.getTotalEventPriceBeforeDiscounts(totalFB,entertainmentPrice);
                System.out.println(String.format("Your total price is $%d.\nFood Selection: %s\nBeverage Selection: %s\nGuest Count: %d\nEntertainment Selection: %s\n" , totalPricePrediscount, newEvent.foodSelection, newEvent.beverageSelection, newEvent.numOfGuests, newEvent.entertainmentSelection));
